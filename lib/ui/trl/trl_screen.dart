  ...
  Widget build(BuildContext context) {
    return Layouts.layout(Consumer<TrlProvider>(builder: (_, trlRef, __) {
      ...
      childrens.add(Wrap(
          ...
          children: <Widget>[
            trl > 1
                ? Buttons.bt1("TRL ${trl - 1}",
                    hardware ? Colors.blue.shade900 : Colors.green.shade900,
                    () {
                    trlRef.setTrl(trl - 1);
                  })
                : SizedBox(width: 100),
      ...
      for (var i = 0; i < TrlModel.getNumberSecIssues(hardware, trl); i++) {
        childrens.add(Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: <Widget>[
              Expanded(
                child: CheckboxListTile(
                  title: Text(TrlModel.getSecIssue(hardware, trl, i + 1)),
